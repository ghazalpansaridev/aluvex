...
export function OrderVolumeChart({ data, loading, periodLabel }: OrderVolumeChartProps) {
  ...
  const title = periodLabel ? `Order Volume (${periodLabel})` : 'Order Volume';
        ...
        <BarChart
          ...
          barInnerComponent={(item: { value: number }) => (
            <View style={styles.barInnerLabel}>
              <Text style={styles.barInnerLabelText} numberOfLines={1}>
                {item.value}
              </Text>
            </View>
          )}
